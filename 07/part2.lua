...
local disk_size = 70000000
local size_target = 30000000

local available = disk_size - dirs['rootbaby']
local space_to_remove = size_target - available

local smallest_dir_size = nil

for _, v in pairs(dirs) do
    if v >= space_to_remove then
        if smallest_dir_size == nil or v < smallest_dir_size then
            smallest_dir_size = v
        end
    end
end
